...
def acasxu_model_perturb(fpath:str = None):
    import readNNet,acas_nnet2pth
    """Load ACASXU model from nnet file.
        Args: fpath: path to the nnet file, such as ACASXU_experimental_v2a_perturb_rate0.01_1_2.nnet"""
    if fpath is not None:
        fpath_full = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'models', 'acas_xu_random', fpath)
    model = acas_nnet2pth.nnet2pth(fpath_full)
    return model
...
def acas_xu_robustness_properties(eps):
    '''
    write the robustness properties for one nnet, the dataset is builtin.
    :param radius_list: a list of radius for each data 1~5 
    :param labels: a list of labels for each data 1~5'''
    # data_1 = [0, 0, 0, 0, 0 ]

    # data_2 = [0.285572907851,0.302394391875,-0.499602112642,0,0]

    # data_3 = [0.45, -0.23, -0.4, 0.12, 0.33 ]

    # data_4 = [ -0.2, -0.25, -0.5, -0.3, -0.44 ]

    # data_5 = [ 0.61, 0.36, 0.0, 0.0, -0.24 ]

    radius_5_2 = [0.0023437500000000003,
              0.16640625000000003,
              0.11015625,
              0.05546875,
              0.06640625]

    labels_5_2 = [4,0,0,0,0]

    radius_list = radius_5_2
    labels = labels_5_2

    X = torch.tensor([[0, 0, 0, 0, 0 ],
                      [0.285572907851,0.302394391875,-0.499602112642,0,0],
                      [0.45, -0.23, -0.4, 0.12, 0.33 ],
                      [ -0.2, -0.25, -0.5, -0.3, -0.44 ],
                      [ 0.61, 0.36, 0.0, 0.0, -0.24 ]]).float()
    
    radius = torch.tensor(radius_list).float().reshape(-1, 1)
