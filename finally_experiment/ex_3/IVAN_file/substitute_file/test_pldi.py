...
# test acasxu instances
class TestAAAI(TestCase):
    def test(self):
        args = pt.TransferArgs(net=config.ACASXU_randomized_torch(), domain=Domain.DEEPZ, approx=ap.aaai(),
                               dataset=Dataset.AAAI, split=Split.INPUT_SB, timeout=20000)
        pt.proof_transfer_AAAI(args)
